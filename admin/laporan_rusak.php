...
<head>
 <?php 
$per_hal=5;
$jumlah_record=mysql_query("SELECT COUNT(*) from view_balik");
...
<div class="col-md-12">
 ...
 <form action="cari_pinjam.php" method="get">
...
<table class="table table-bordered table-striped table-hover" >
 ...
 <?php 
  if(isset($_GET['cari'])){
    ...
    $brg=mysql_query("select * from view_balik where nama like '$cari%' ");
  }else{
    $brg=mysql_query("select * from view_balik limit $start, $per_hal");
  }
  $no=1;
  while($data=mysql_fetch_array($brg)){
    ...
    ?>    
            ...
            <td class="center"><a href="kembali_rusak.php?id=<?= $data['id_out']; ?>&jumlah=<?= $data['jumlah_out'];?>&idinv=<?= $data['id_inventaris'];?>" class="btn btn-success" onClick = "return confirm('Apakah Anda ingin mengembalikan barang rusak  <?php echo $data['nama']; ?>?')"> Kembalikan </a></td>
                                        </tr>
